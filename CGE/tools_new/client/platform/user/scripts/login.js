...
function showLoggedin(uid, sesid){
   ...
   window.user_logged_in = true;
	...
	// DISPLAYING LOGGED IN ELEMENT AND HIDING LOGIN <a href='javascript:void(0)'> </a>
	document.getElementById('login').innerHTML = "\
         <ul class='topnav'>\
            <li style='font-size:16px;'>Welcome <span>"+ uid +"</span>\
               <ul class='subnav'>\
                  <li><a href='/services/CGEpipeline-1.0/map.php'>Map Overview</a></li>\
                  <li><a href='/services/cge/'>Batch Uploader</a></li>\
                  <li><a href='/tools_new/client/platform/sample/'>Sample Manager</a></li>\
                  <li><a href='/services/cge/submitservice.php'>Run Analysis</a></li>\
...
function showLogin(){
   ...
   window.user_logged_in = false;
	...
	<form> \
		<div> \
			<span class='login' style='margin: 0 2 0 0;'>Username</span> \
			<span><input type='text' name='username' value='' style='width: 65px;height:14px;font-size: 10px;'></span> \
		</div><div> \
			<span class='login' style='margin: 0 5 0 0;'>Password</span> \
			<span><input type='password' name='password' value='' style='width: 65px;height:14px;font-size: 10px;'></span> \
		</div><div> \
			<!-- <input class='login' type='button' value='New' onclick='window.open(&quot;/cge/user/login/user_manager.php?action=create&quot;)' style='width: 34px;height:18px;font-size: 10px;text-align: center;'> -->\
			<input class='login' type='button' value='Reset' onclick='showReset();' style='width: 40px;height:18px;font-size: 10px;text-align: center;'> \
			<input class='login' type='button' value='Login' onclick='login(this.form.username.value, this.form.password.value);' style='width: 39px;height:18px;font-size: 10px;text-align: center;'> \
		</div> \
	</form>";
}
    ...
    * REMOVING LOG IN ELEMENT
	...
	<form> \
		<div> \
			...
			<input type='button' value='Reset Password' onclick='reset_user(this.form.email.value);' style='width: 85px;height:17px;font-size: 9px;'> \
		</div> \
	</form>";
}
