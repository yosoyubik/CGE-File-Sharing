...
if (count($_POST)+count($_GET)>0 and isset($_SESSION['SESSIONID']) and isset($_SESSION['USERNAME'])){
   ...
   if ($fid != null){
      ...
      //CLOSING DATABASE
      $mysqli->close();
   }elseif ($un != null && $iid != null){
      // VALIDATE INPUTS
      if ($iid == ''){ respond("NOIID", '', true); }
      if ($un == ''){ respond("NOUN", '', true); }
      if (preg_match("/[^0-9]/", $iid)){ respond("BADIID", '', true); }
      if (preg_match("/[^0-9]/", $un)){ respond("BADUN", '', true); }
      // PROCESS INPUTS
      ...
      if($status=="ACCEPTED"){	
         // RETRIEVE FILE PATH FROM DATABASE
         $stmt = $mysqli->prepare("SELECT file_names
                                   FROM isolates
                                   WHERE id = ?
                                   AND owner = ?
                                   ;");
         ...
         // RESPOND
         if ($count >= 1){
            // GET filePath and fileName from file_names
            $fullpath = explode(',', $file_names)[+$un];
            $path_parts = pathinfo($fullpath);
            $filePath = trim($path_parts['dirname']);
            $filename = trim($path_parts['basename']);
            //respond($status, "#$filePath#  #$filename#", true);
         }else{
            // RESPOND WITH NO DATA FOUND
            respond('NODATA', '');
         }
