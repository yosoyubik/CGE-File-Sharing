...
function login(uid, pwd){
    ...
    */
	if(uid != '' & pwd != ''){
		$.ajax({
			type : "POST",
			url : "https://cge.cbs.dtu.dk/cge/user/login/php/login.php",
			data : {
				USERNAME : uid,
				PASSWORD : pwd
			},
...
function logout(){
    ...
    */
	$.ajax({
		type : "POST",
		url : "https://cge.cbs.dtu.dk/cge/user/login/php/logout.php",
		data : {},
		timeout : 2000,
		dataType : 'xml',
		async : false,
...
function reset_user(email){
    ...
    */
	if(email != ''){
		$.ajax({
			type : "POST",
			url : "https://cge.cbs.dtu.dk/cge/user/login/php/reset_user.php",
			data : {
				EMAIL : email
         },
			timeout : 2000,
         ...
         <ul class='topnav'>\
            <li>Welcome <span><a href='#'>"+ uid +"</a></span>\
               <ul class='subnav'>\
                  ...
                  <li><a href='/cge/user/isolate/isolate_manager.php'>Manage Isolates</a></li>\
                  <li><a href='/cge/user/login/user_manager.php?action=show'>Settings</a></li>\
...
function showLogin(){
   ...
   window.user_logged_in = false;
	...
	<form> \
		<div style='width:110px;margin:2 0;text-align: right;'> \
		...
		</div><div  style='width:120px;margin: auto;text-align: center;'> \
			...
			<input type='button' value='Reset' onclick='showReset();' style='width: 40px;height:17px;font-size: 8px;text-align: center;'> \
...
function showReset(){
    ...
    * REMOVING LOG IN ELEMENT
	...
	document.getElementById('reset').innerHTML = "\
	<form> \
		<div style='width:130px;margin:2 0;text-align: right;'> \
			<span style='margin: 0 5 0 0;'>Email</span> \
			<span><input type='text' name='email' value='' style='width: 90px;height:16px;font-size: 10px;'></span> \
		</div><div  style='width:130px;margin:2 3;text-align: center;'> \
