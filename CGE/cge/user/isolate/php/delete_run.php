...
// MAIN
if ((count($_POST)>0 or count($_GET)>0)  and isset($_SESSION['SESSIONID']) and isset($_SESSION['USERNAME'])){ # count($_POST)>0 or count($_GET)>0
	// VALIDATE SESSION
   ...
   $USERNAME = $_SESSION['USERNAME'];
	...
	// VALIDATE INPUTS
   $SID = _INPUT("SID");
	if (preg_match("/[^0-9]/", $SID)){ respond("BADID", '', true); }
	if ($SID == ''){ respond("NOID", '', true); }
   ...
   // CHECK USERNAME, SESSIONID AND ISOLATE
	...
	$stmt->execute();
	$stmt->bind_result($uid, $sesid, $ifolder, $sfolder);
