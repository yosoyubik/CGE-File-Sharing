...
function create_user(uid, pwd, email, captcha){
   ...
   // SEND REQUEST FOR USER CREATION
	if(uid != '' & pwd != '' & email != '' & captcha != ''){
		$.ajax({
			type : "POST",
			url : "/cge/user/login/php/create_user.php",
			data : {
				USERNAME : uid,
				PASSWORD : pwd,
            EMAIL    : email,
...
function update_user(uid, pwd, email){
   ...
   if(!check_name(uid) | !check_email(email) | !check_password()){
	...
	if(uid != '' & pwd != '' & email != ''){
		$.ajax({
			type : "POST",
			url : "/cge/user/login/php/update_user.php",
			data : {
				USERNAME : uid,
				PASSWORD : pwd,
            EMAIL    : email,
...
function delete_user(){
   ...
   if(confirm('Are you sure you wish to delete this profile and all of it non-public data?')){
	   $.ajax({
	   	type : "POST",
	   	url : "/cge/user/login/php/delete_user.php",
	   	data : {},
	   	timeout : 2000,
	   	dataType : 'xml',
	   	async : false,
...
function get_user_details(){
    ...
    */
	$.ajax({
		type : "POST",
		url : "/cge/user/login/php/get_user_details.php",
		data : {},
		timeout : 2000,
		dataType : 'xml',
		async : false,
